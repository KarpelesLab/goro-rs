...
fn md5_fn(_vm: &mut Vm, _args: &[Value]) -> Result<Value, VmError> {
    Ok(Value::String(PhpString::empty()))
}
fn sha1_fn(_vm: &mut Vm, _args: &[Value]) -> Result<Value, VmError> {
    Ok(Value::String(PhpString::empty()))
}
...
fn ctype_alpha(_vm: &mut Vm, args: &[Value]) -> Result<Value, VmError> {
    let s = args.first().unwrap_or(&Value::Null).to_php_string();
    Ok(
        if !s.is_empty() && s.as_bytes().iter().all(|b| b.is_ascii_alphabetic()) {
            Value::True
        } else {
            Value::False
        },
...
fn ctype_alnum(_vm: &mut Vm, args: &[Value]) -> Result<Value, VmError> {
    let s = args.first().unwrap_or(&Value::Null).to_php_string();
    Ok(
        if !s.is_empty() && s.as_bytes().iter().all(|b| b.is_ascii_alphanumeric()) {
            Value::True
        } else {
            Value::False
        },
...
fn ctype_upper(_vm: &mut Vm, args: &[Value]) -> Result<Value, VmError> {
    let s = args.first().unwrap_or(&Value::Null).to_php_string();
    Ok(
        if !s.is_empty() && s.as_bytes().iter().all(|b| b.is_ascii_uppercase()) {
            Value::True
        } else {
            Value::False
        },
...
fn ctype_lower(_vm: &mut Vm, args: &[Value]) -> Result<Value, VmError> {
    let s = args.first().unwrap_or(&Value::Null).to_php_string();
    Ok(
        if !s.is_empty() && s.as_bytes().iter().all(|b| b.is_ascii_lowercase()) {
            Value::True
        } else {
            Value::False
        },
...
fn ctype_space(_vm: &mut Vm, args: &[Value]) -> Result<Value, VmError> {
    let s = args.first().unwrap_or(&Value::Null).to_php_string();
    Ok(
        if !s.is_empty() && s.as_bytes().iter().all(|b| b.is_ascii_whitespace()) {
            Value::True
        } else {
            Value::False
        },
